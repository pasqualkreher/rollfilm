...
export function ZoomReadout({ zoom }: { zoom: ZoomReadoutTarget }) {
  ...
  return (
    <span className="zoom-readout">
      ...
      <span className="zoom-readout-value" title="Current zoom, in actual pixels">
        {percent === null ? "—" : `${percent}%`}
      </span>
      <span className="zoom-readout-hint">
        {zoom.zoomed ? "Drag to pan · Esc to fit" : "Scroll, pinch or double-click to zoom"}
      </span>
    </span>
  );
}
