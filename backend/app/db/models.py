...
class CanvasLayout(Base):
    ...
    # Page margins: drawn as a guide on each sheet, a snap target, and where
    # the auto-layout flows photos. Two values, not one: margin_mm is the
    # left and right margin (its name predates the split), margin_y_mm the
    # top and bottom one. 12mm is what the layout always used.
    margin_mm: Mapped[float] = mapped_column(Float, default=12.0, server_default="12")
    margin_y_mm: Mapped[float] = mapped_column(Float, default=12.0, server_default="12")
